---
title: "Biomass analysis: no/low P treatment"
...
setwd("~/Desktop/MASTER Pflanzenwissenschaften/Experiments/Magenta assay/Magenta LOW:NO")


data<-read.csv(file="Magenta_no:lowP.csv", header=TRUE, sep=",")
...
HPC<- subset(data_norm, treatment=="noP_control")
LPS<- subset(data_norm, treatment=="lowP_SynCom9")
HPS<- subset(data_norm, treatment=="noP_SynCom9")
...
summary(A2)
A3<- aov(roots_fresh_weight~treatment*SynCom9, data=data_norm)
summary(A3)
A4<- aov(roots_dry_weight~treatment*SynCom9, data=data_norm)
summary(A4)
...
```{r, ggplot_shoot_fresh_weight ,echo=F, message=F, warning=F}
...
F1<-data_norm %>%
  ...
  theme_classic()+
  guides(fill="none")+
  geom_text(data=Tk1, aes(label=cld1,x=treatment, y=quant),vjust=-2.5,hjust=-0.5, size=4)+
  scale_fill_manual(values = c("lightskyblue1","cornflowerblue","goldenrod1", "coral"))+
  scale_shape_manual(values = c(16,17),labels=c("A1","A2"))+
  labs(y= "shoot fresh weight (z-transformed)")+
  scale_x_discrete(labels=c("LP C", " LP Syn9", "NP C", "NP Syn9"))+
...
F1
ggsave("boxplot_shoot_fresh_weight_no_low.png", width=6, height=4) 
```
...
```{r, ggplot_shoot_dry_weight,echo=F, message=F, warning=F}
#shoot dry weight
F2<-data_norm %>%
  ...
  theme_classic()+
  guides(fill="none")+
  geom_text(data=Tk2, aes(label=cld2,x=treatment, y=quant),vjust=-3.9,hjust=-0.5, size=4)+
  scale_fill_manual(values = c("lightskyblue1","cornflowerblue","goldenrod1", "coral"))+
  scale_shape_manual(values = c(16,17),labels=c("A1","A2"))+
...
F2
ggsave("boxplot_shoot_dry_weight_no_low.png",width=6, height=4 ) 
```
...
F3<-data_norm %>%
  ...
  theme_classic()+
  guides(fill="none")+
  geom_text(data=Tk3, aes(label=cld3,x=treatment, y=quant),vjust=-2.5,hjust=-0.5, size=4)+
  scale_fill_manual(values = c("lightskyblue1","cornflowerblue","goldenrod1", "coral"))+
  scale_shape_manual(values = c(16),labels=c("A1"))+
  labs(y= "shoot dry weight (z-transformed)")+
  scale_x_discrete(labels=c("LP C", " LP Syn9", "NP C", "NP Syn9"))+
  scale_y_continuous(limits = c(-2, 3))+
  theme(legend.position = "none")+
...
F3
ggsave("boxplot_roots_fresh_weight_no_low.png",width=6, height=4 ) 
```
...
F4<-data_norm %>%
  ...
  theme_classic()+
  guides(fill="none")+
  geom_text(data=Tk4, aes(label=cld4,x=treatment, y=quant),vjust=-2.7,hjust=-0.5, size=4)+
  scale_fill_manual(values = c("lightskyblue1","cornflowerblue","goldenrod1", "coral"))+
  scale_shape_manual(values = c(16),labels=c("A1"))+
  labs(y= "shoot dry weight (z-transformed)")+
  scale_x_discrete(labels=c("LP C", " LP Syn9", "NP C", "NP Syn9"))+
...
F4
ggsave("boxplot_roots_dry_weight_no_low.png",width=6, height=4) 
```
...
ggsave("figure.png",width=6, height=6) 
